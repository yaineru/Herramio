...
const TOOL_ID = "imagen-eliminar-fondo-color";
...
export function ImageBackgroundRemover() {
  ...
  useEffect(() => {
    ...
    const ctx = canvas.getContext("2d");
    if (!ctx) return;
    const out = removeColorBackground(original.data, targetColor, tolerance);
    ctx.putImageData(new ImageData(new Uint8ClampedArray(out), canvas.width, canvas.height), 0, 0);
    setApplied(true);
  }, [targetColor, tolerance, picking]);
  ...
  async function handleDownload() {
    ...
    downloadBlob(blob, `${base}-sin-fondo.png`);
  ...
  async function handleSendToCompressor() {
    ...
    if (!canvas) return;
    const blob = await canvasToBlob(canvas, "image/png");
    const handoffFile = new File([blob], "sin-fondo.png", { type: "image/png" });
  ...
  return (
    <Card className="p-6">
      <p className="mb-3 flex items-center gap-2 text-sm text-slate-500">
        ...
        {picking ? "Haz clic en el color de fondo que quieres eliminar." : "Ajusta la tolerancia o elige otro color."}
      </p>
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-medium uppercase tracking-wide text-slate-400">Color seleccionado</p>
