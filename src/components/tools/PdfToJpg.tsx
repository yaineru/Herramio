...
export function PdfToJpg() {
  ...
  useEffect(() => {
    AnalyticsEvents.toolOpened("pdf-a-jpg");
  }, []);
  ...
  async function handleConvert() {
    ...
    try {
      ...
      for (const zeroBasedPage of pages) {
        const blob = await renderPdfPageToBlob(file, zeroBasedPage + 1);
        rendered.push({ page: zeroBasedPage + 1, blob, url: URL.createObjectURL(blob) });
      }
      setResults(rendered);
      AnalyticsEvents.toolUsed("pdf-a-jpg");
    } catch {
      const message = "No se pudo convertir el PDF a imágenes.";
      setError(message);
      AnalyticsEvents.toolError("pdf-a-jpg", message);
    } finally {
  ...
  function handleDownload(image: PageImage) {
    downloadBlob(image.blob, `pagina-${image.page}.jpg`);
    AnalyticsEvents.toolDownloaded("pdf-a-jpg", "jpg");
  }
  ...
  return (
    <Card className="p-6">
      ...
      {pageCount !== null && (
        <div className="mt-6">
          <Label htmlFor="pdf-jpg-ranges">Páginas a convertir</Label>
          <Input
            id="pdf-jpg-ranges"
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        <Button type="button" onClick={handleConvert} disabled={!pageCount || isProcessing}>
          ...
          {isProcessing ? "Convirtiendo…" : "Convertir a JPG"}
        </Button>
