...
import { ArrowRight, Copy, Download, FileWarning, Loader2, RotateCcw } from "lucide-react";
...
const TOOL_ID = "pdf-duplicar-pagina";
...
export function PdfDuplicatePage() {
  ...
  async function handleDuplicate() {
    ...
    if (!Number.isInteger(pageNumber) || pageNumber < 1 || pageNumber > pageCount) {
      setError(`Escribe una página entre 1 y ${pageCount}.`);
      return;
    }
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : "No se pudo duplicar la página.";
      setError(message);
  ...
  function handleDownload() {
    ...
    downloadBlob(resultBlob, "documento-con-pagina-duplicada.pdf");
    AnalyticsEvents.toolDownloaded(TOOL_ID, "pdf");
  }

  function handleSendToNumberer() {
    if (!resultBlob) return;
    const handoffFile = new File([resultBlob], "documento-con-pagina-duplicada.pdf", { type: "application/pdf" });
  ...
  return (
    <Card className="p-6">
      ...
      {pageCount !== null && (
        <div className="mt-6">
          <Label htmlFor="duplicate-page">Página a duplicar</Label>
          <Input
            id="duplicate-page"
            type="number"
            inputMode="numeric"
          ...
          <p className="mt-1 text-xs text-slate-400">
            Un valor entre 1 y {pageCount}. La copia se inserta justo después del original.
          </p>
        </div>
      )}
      ...
      {resultBlob && (
        <div className="mt-6 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3">
          <p className="text-sm text-emerald-800">PDF listo ({formatBytes(resultBlob.size)}, {pageCount !== null ? pageCount + 1 : "?"} páginas).</p>
        </div>
      )}

      <div className="mt-6 flex flex-wrap gap-2">
        <Button type="button" onClick={handleDuplicate} disabled={!pageCount || !pageInput || isProcessing}>
          {isProcessing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Copy className="h-4 w-4" />}
          {isProcessing ? "Duplicando…" : "Duplicar página"}
        </Button>
        {resultBlob && (
