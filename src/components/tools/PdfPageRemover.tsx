...
import { ArrowRight, Download, FileWarning, Loader2, RotateCcw, Trash2 } from "lucide-react";
...
const TOOL_ID = "pdf-eliminar-paginas";
...
export function PdfPageRemover() {
  ...
  async function handleRemove() {
    ...
    setError(null);
    try {
      const blob = await removePdfPages(file, toRemove);
      setResultBlob(blob);
      AnalyticsEvents.toolUsed(TOOL_ID);
    } catch {
      const message = "No se pudieron eliminar las páginas.";
      setError(message);
  ...
  function handleDownload() {
    ...
    downloadBlob(resultBlob, "documento-sin-paginas.pdf");
    AnalyticsEvents.toolDownloaded(TOOL_ID, "pdf");
  }

  function handleSendToSplitter() {
    if (!resultBlob) return;
    const handoffFile = new File([resultBlob], "documento-sin-paginas.pdf", { type: "application/pdf" });
  ...
  return (
    <Card className="p-6">
      ...
      {pageCount !== null && (
        <div className="mt-6">
          <Label htmlFor="pages-to-remove">Páginas a eliminar</Label>
          <Input
            id="pages-to-remove"
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        <Button type="button" onClick={handleRemove} disabled={!pageCount || !rangeInput || isProcessing}>
          {isProcessing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
          {isProcessing ? "Eliminando…" : "Eliminar páginas"}
        </Button>
        {resultBlob && (
