...
import { ArrowRight, Download, FileWarning, Loader2, RotateCcw, Stamp } from "lucide-react";
...
import { MAX_PDF_BYTES, getPdfPageCount, addWatermark } from "@/lib/pdf/pdf-engine";
import { setToolHandoff } from "@/lib/tool-handoff";
import { AnalyticsEvents } from "@/lib/analytics";

const TOOL_ID = "pdf-marca-agua";
...
export function PdfWatermark() {
  ...
  async function handleApply() {
    if (!file || !pageCount || !text.trim()) return;
    setIsProcessing(true);
    setError(null);
    try {
      const blob = await addWatermark(file, text.trim(), opacity / 100, fontSize);
      setResultBlob(blob);
      AnalyticsEvents.toolUsed(TOOL_ID);
    } catch {
      const message = "No se pudo aplicar la marca de agua.";
      setError(message);
  ...
  function handleDownload() {
    ...
    downloadBlob(resultBlob, "documento-marca-agua.pdf");
    AnalyticsEvents.toolDownloaded(TOOL_ID, "pdf");
  }

  function handleSendToSplitter() {
    if (!resultBlob) return;
    const handoffFile = new File([resultBlob], "documento-marca-agua.pdf", { type: "application/pdf" });
    ...
    <Card className="p-6">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-3">
        <div className="sm:col-span-3">
          <Label htmlFor="watermark-pdf-text">Texto de la marca de agua</Label>
          <Input id="watermark-pdf-text" value={text} onChange={(e) => setText(e.target.value)} />
        </div>
        <div>
          <Label htmlFor="watermark-pdf-opacity">Opacidad ({opacity}%)</Label>
          <input id="watermark-pdf-opacity" type="range" min={5} max={80} value={opacity} onChange={(e) => setOpacity(Number(e.target.value))} className="mt-2 w-full accent-emerald-600" />
        </div>
        <div className="sm:col-span-2">
          <Label htmlFor="watermark-pdf-size">Tamaño de letra ({fontSize}pt)</Label>
          <input id="watermark-pdf-size" type="range" min={20} max={90} value={fontSize} onChange={(e) => setFontSize(Number(e.target.value))} className="mt-2 w-full accent-emerald-600" />
        </div>
      </div>
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        <Button type="button" onClick={handleApply} disabled={!pageCount || !text.trim() || isProcessing}>
          {isProcessing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Stamp className="h-4 w-4" />}
          {isProcessing ? "Aplicando…" : "Añadir marca de agua"}
        </Button>
        {resultBlob && (
