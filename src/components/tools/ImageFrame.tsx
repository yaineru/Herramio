...
import { ArrowRight, Download, Frame, ImageOff, RotateCcw } from "lucide-react";
...
export function ImageFrame() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      const border = Math.round(Math.min(loaded.width, loaded.height) * (widthPercent / 100));
      const canvas = document.createElement("canvas");
      canvas.width = loaded.width + border * 2;
      canvas.height = loaded.height + border * 2;
      const ctx = canvas.getContext("2d");
      if (!ctx) return;

      ctx.fillStyle = color;
      ctx.fillRect(0, 0, canvas.width, canvas.height);
      ctx.drawImage(loaded.img, border, border);
  ...
  }, [loaded, widthPercent, color, file]);
  ...
  function handleDownload() {
    ...
    downloadBlob(resultBlob, `${base}-con-marco.${ext}`);
  ...
  function handleSendToCompressor() {
    if (!resultBlob || !file) return;
    const type = file.type === "image/png" ? "image/png" : "image/jpeg";
    const handoffFile = new File([resultBlob], `marco.${type === "image/png" ? "png" : "jpg"}`, { type });
    ...
    <Card className="p-6">
      <div className="flex min-h-56 items-center justify-center overflow-hidden rounded-xl border border-slate-200 bg-slate-100 p-4">
        {resultUrl ? (
          ...
          <img src={resultUrl} alt="Vista previa con marco" className="max-h-96 max-w-full object-contain" />
        ) : loaded ? (
          // eslint-disable-next-line @next/next/no-img-element
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        <div>
          <Label htmlFor="frame-width">Ancho del marco ({widthPercent}%)</Label>
          <input
            id="frame-width"
            type="range"
            min={1}
        ...
        <div>
          <Label htmlFor="frame-color">Color del marco</Label>
          <input
            id="frame-color"
            type="color"
            value={color}
      ...
      <p className="mt-4 flex items-center gap-1.5 text-xs text-slate-400">
        <Frame className="h-3 w-3" /> Tu imagen se procesa directamente en tu navegador: no se sube a nuestros
