...
import { ArrowRight, Download, FileWarning, Loader2, RotateCcw, Shrink } from "lucide-react";
...
export function PdfCompressor() {
  ...
  function handleFiles(files: File[]) {
    ...
    getPdfPageCount(picked)
      .then((count) => {
        if (count > MAX_PAGES) {
          setError(`Este PDF tiene ${count} páginas; el compresor admite hasta ${MAX_PAGES} por archivo.`);
          return;
        }
        setPageCount(count);
      })
  ...
  async function handleCompress() {
    ...
    try {
      ...
      for (let page = 1; page <= pageCount; page++) {
        const blob = await renderPdfPageToBlob(file, page, 1.5, quality / 100);
        images.push({ file: new File([blob], `pagina-${page}.jpg`, { type: "image/jpeg" }), type: "image/jpeg" });
      ...
      const message = "No se pudo comprimir el PDF.";
      setError(message);
  ...
  function handleDownload() {
    ...
    downloadBlob(resultBlob, "documento-comprimido.pdf");
    AnalyticsEvents.toolDownloaded(TOOL_ID, "pdf");
  }

  function handleSendToSplitter() {
    if (!resultBlob) return;
    const handoffFile = new File([resultBlob], "documento-comprimido.pdf", { type: "application/pdf" });
  ...
  return (
    <Card className="p-6">
      ...
      {pageCount !== null && (
        <div className="mt-6">
          <Label htmlFor="compress-quality">Calidad de compresión ({quality}%)</Label>
          <input
            id="compress-quality"
            type="range"
            min={20}
          ...
          <p className="mt-1 text-xs text-slate-400">
            Menos calidad = archivo más pequeño. Ideal para PDFs escaneados o con muchas imágenes.
          </p>
        </div>
      )}
      ...
      {isProcessing && progress && (
        <p className="mt-4 text-sm text-slate-500">Comprimiendo página {progress.done} de {progress.total}…</p>
      )}
      ...
      {resultBlob && (
        <div className="mt-6 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3">
          <p className="text-sm text-emerald-800">
            ...
            {reduction !== null && reduction > 0 && <span className="ml-1.5 font-medium">(−{reduction}%)</span>}
            {reduction !== null && reduction <= 0 && <span className="ml-1.5">sin reducción de tamaño</span>}
          </p>
        </div>
      )}

      <div className="mt-6 flex flex-wrap gap-2">
        <Button type="button" onClick={handleCompress} disabled={!pageCount || isProcessing}>
          {isProcessing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Shrink className="h-4 w-4" />}
          {isProcessing ? "Comprimiendo…" : "Comprimir PDF"}
        </Button>
        {resultBlob && (
      ...
      <p className="mt-4 text-xs text-slate-400">
        ...
        este proceso. Cada página se convierte en una imagen comprimida, así que el texto dejará de ser
        seleccionable — ideal para documentos escaneados o con muchas fotos.
      </p>
    </Card>
  );
}
