...
import { ArrowRight, CloudFog, Download, ImageOff, RotateCcw } from "lucide-react";
...
export function ImageShadow() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      const padding = Math.round(blur * 2 + Math.abs(offsetY) + 20);
      const canvas = document.createElement("canvas");
      canvas.width = loaded.width + padding * 2;
      canvas.height = loaded.height + padding * 2;
      ...
      ctx.drawImage(loaded.img, padding, padding);
  ...
  function handleDownload() {
    ...
    downloadBlob(resultBlob, `${base}-con-sombra.png`);
    AnalyticsEvents.toolDownloaded(TOOL_ID, "png");
  }

  function handleSendToCompressor() {
    if (!resultBlob) return;
    const handoffFile = new File([resultBlob], "con-sombra.png", { type: "image/png" });
    ...
    return (
      <Card className="p-6">
        <FileDropZone
          ...
          hint={`Ideal con imágenes PNG de fondo transparente — máx. ${formatBytes(MAX_IMAGE_BYTES)}`}
        />
        {error && (
    ...
    <Card className="p-6">
      <div className="flex min-h-56 items-center justify-center overflow-hidden rounded-xl border border-slate-200 bg-slate-100 p-4">
        {resultUrl ? (
          ...
          <img src={resultUrl} alt="Vista previa con sombra" className="max-h-96 max-w-full object-contain" />
        ) : loaded ? (
          // eslint-disable-next-line @next/next/no-img-element
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-3">
        <div>
          <Label htmlFor="shadow-blur">Difuminado ({blur}px)</Label>
          <input id="shadow-blur" type="range" min={0} max={80} value={blur} onChange={(e) => setBlur(Number(e.target.value))} className="mt-2 w-full accent-emerald-600" />
        </div>
        <div>
          <Label htmlFor="shadow-offset">Distancia ({offsetY}px)</Label>
          <input id="shadow-offset" type="range" min={0} max={60} value={offsetY} onChange={(e) => setOffsetY(Number(e.target.value))} className="mt-2 w-full accent-emerald-600" />
        </div>
        <div>
          <Label htmlFor="shadow-opacity">Opacidad ({opacity}%)</Label>
          <input id="shadow-opacity" type="range" min={5} max={90} value={opacity} onChange={(e) => setOpacity(Number(e.target.value))} className="mt-2 w-full accent-emerald-600" />
        </div>
      </div>
      ...
      <p className="mt-4 flex items-center gap-1.5 text-xs text-slate-400">
        <CloudFog className="h-3 w-3" /> Si tu imagen tiene fondo blanco (no transparente), la sombra se verá sobre
        ese fondo blanco. Para mejores resultados, usa primero Eliminar Fondo de Color. Todo se procesa en tu
        navegador.
      </p>
    </Card>
  );
