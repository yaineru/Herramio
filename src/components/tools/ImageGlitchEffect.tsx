...
import { ArrowRight, Download, ImageOff, RotateCcw, Zap } from "lucide-react";
...
const TOOL_ID = "imagen-efecto-glitch";

export function ImageGlitchEffect() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      ...
      const glitched = applyGlitchEffect(original.data, canvas.width, canvas.height, intensity);
      ctx.putImageData(new ImageData(new Uint8ClampedArray(glitched), canvas.width, canvas.height), 0, 0);
  ...
  function handleDownload() {
    ...
    downloadBlob(resultBlob, `${base}-glitch.jpg`);
    AnalyticsEvents.toolDownloaded(TOOL_ID, "jpg");
  }

  function handleSendToWatermark() {
    if (!resultBlob) return;
    const handoffFile = new File([resultBlob], "glitch.jpg", { type: "image/jpeg" });
    ...
    <Card className="p-6">
      <div className="flex min-h-56 items-center justify-center overflow-hidden rounded-xl border border-slate-200 bg-slate-100 p-4">
        {resultUrl ? (
          ...
          <img src={resultUrl} alt="Vista previa con efecto glitch" className="max-h-96 max-w-full object-contain" />
        ) : loaded ? (
          // eslint-disable-next-line @next/next/no-img-element
      ...
      <div className="mt-5">
        <Label htmlFor="glitch-intensity">Intensidad ({intensity})</Label>
        <input
          id="glitch-intensity"
          type="range"
          min={0}
      ...
      <p className="mt-4 flex items-center gap-1.5 text-xs text-slate-400">
        <Zap className="h-3 w-3" /> Efecto real de separación de canales de color y desplazamiento de franjas —
        procesado directamente en tu navegador, sin subir nada a un servidor.
      </p>
    </Card>
  );
}
