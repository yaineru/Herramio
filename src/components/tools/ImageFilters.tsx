...
import { DEFAULT_FILTER_SETTINGS, FILTER_PRESETS, buildFilterString, type ImageFilterSettings } from "@/lib/images/filters";
import { setToolHandoff } from "@/lib/tool-handoff";
import { AnalyticsEvents } from "@/lib/analytics";
import { cn } from "@/lib/utils";

const TOOL_ID = "imagen-filtros";

const SLIDERS: { key: keyof ImageFilterSettings; label: string; min: number; max: number }[] = [
  { key: "grayscale", label: "Blanco y negro", min: 0, max: 100 },
  ...
  { key: "invert", label: "Invertir colores", min: 0, max: 100 },
];
...
export function ImageFilters() {
  ...
  function applyPreset(presetSettings: Partial<ImageFilterSettings>) {
    setSettings({ ...DEFAULT_FILTER_SETTINGS, ...presetSettings });
  }
  ...
  function handleDownload() {
    ...
    downloadBlob(resultBlob, `${base}-filtro.${ext}`);
  ...
  function handleSendToCompressor() {
    if (!resultBlob || !file) return;
    const type = file.type === "image/png" ? "image/png" : "image/jpeg";
    const handoffFile = new File([resultBlob], `filtro.${type === "image/png" ? "png" : "jpg"}`, { type });
    ...
    <Card className="p-6">
      <div className="flex min-h-56 items-center justify-center overflow-hidden rounded-xl border border-slate-200 bg-slate-100 p-4">
        {resultUrl ? (
          ...
          <img src={resultUrl} alt="Vista previa con filtro" className="max-h-96 max-w-full object-contain" />
        ) : loaded ? (
          // eslint-disable-next-line @next/next/no-img-element
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        {SLIDERS.map(({ key, label, min, max }) => (
          <div key={key}>
            <Label htmlFor={`filter-${key}`}>{label} ({settings[key]})</Label>
            <input
              id={`filter-${key}`}
              type="range"
              min={min}
              ...
              className={cn("mt-2 w-full accent-emerald-600")}
            />
          </div>
        ))}
      </div>
